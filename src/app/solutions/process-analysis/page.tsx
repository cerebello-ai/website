import Link from 'next/link';

import { ArrowRight } from 'lucide-react';
import { Metadata } from 'next';

import { Cta7 } from '@/components/shadcnblocks/cta/cta7';
import { Feature187 } from '@/components/shadcnblocks/feature/feature187';
import { Stats9 } from '@/components/shadcnblocks/stats/stats9';
import { Testimonial8 } from '@/components/shadcnblocks/testimonial/testimonial8';
import { Button } from '@/components/ui/button';

export const metadata: Metadata = {
  title: 'Process Analysis | AI-Powered Business Optimization | Cerebello',
  description:
    'Systematic analysis of business workflows to identify AI automation opportunities and efficiency improvements. Discover hidden inefficiencies through comprehensive process analysis.',
  keywords:
    'Process Analysis, business process optimization, workflow analysis, AI automation opportunities, efficiency improvement, bottleneck identification, Cerebello',
  openGraph: {
    title: 'Process Analysis | Cerebello',
    description:
      'Discover hidden inefficiencies and automation opportunities through comprehensive process analysis.',
    type: 'article',
    url: 'https://cerebello.nl/solutions/process-analysis',
  },
  twitter: {
    card: 'summary_large_image',
    title: 'Process Analysis | Cerebello',
    description:
      'Discover hidden inefficiencies and automation opportunities through comprehensive process analysis.',
  },
  alternates: {
    canonical: 'https://cerebello.nl/solutions/process-analysis',
  },
};

// Process Analysis data
const processAnalysisData = {
  title: 'Process Analysis',
  description:
    'At Cerebello, we help you uncover how work really happens inside your business so you can make it faster, smarter, and ready for automation. Our Process Analysis service dives deep into your operations to identify inefficiencies, map out workflows, and highlight opportunities for AI and automation. The result? Leaner processes, fewer bottlenecks, and better alignment with your strategic goals.',
  introduction:
    "Whether you're dealing with delays, manual work, or unclear handoffs, we bring clarity and actionable insights. We work closely with your team to visualize current workflows, analyze performance, and co-create an improvement roadmap, often pinpointing areas where automation or intelligent tooling can deliver immediate value.",
  content:
    "What sets us apart is our blend of business insight and technical expertise. We don't just recommend change, we design it with your people in mind. Every analysis is practical, data-driven, and built to support long-term transformation. If you're looking to streamline operations, reduce cost, and prepare for scalable AI adoption, it starts here.",
  additionalContent:
    "What sets us apart is our blend of business insight and technical expertise. We don't just recommend change, we design it with your people in mind. Every analysis is practical, data-driven, and built to support long-term transformation. If you're looking to streamline operations, reduce cost, and prepare for scalable AI adoption, it starts here.",
  callToAction:
    "Let's simplify what's slowing you down. Book a free consultation today.",
  benefits: [
    {
      title: 'Improved Efficiency',
      description:
        'Identify redundancies and waste to streamline operations for higher productivity and lower costs',
    },
    {
      title: 'Reduced Bottlenecks',
      description:
        'Pinpoint where work gets stuck and devise solutions for faster cycle times',
    },
    {
      title: 'Data-Driven Insights',
      description:
        'Ground recommendations in metrics and evidence for informed decision-making',
    },
    {
      title: 'Strategic Alignment',
      description:
        'Ensure operations align with broader business objectives and KPIs',
    },
  ],
  methodology: [
    {
      step: '01',
      title: 'Discovery',
      subtitle: 'Business Context Analysis',
      description:
        'Understand your business goals and identify target processes for analysis through stakeholder interviews and documentation review. We combine documented facts with on-the-ground insights to build a complete picture.',
    },
    {
      step: '02',
      title: 'Mapping',
      subtitle: 'Process Visualization',
      description:
        'Create detailed flowcharts and BPMN diagrams to document every step, decision point, and handoff in your workflows. This makes the invisible visible and helps spot bottlenecks.',
    },
    {
      step: '03',
      title: 'Assessment',
      subtitle: 'Performance Evaluation',
      description:
        'Analyze cycle times, throughput, error rates, and other metrics to quantify current performance and identify improvement areas. We establish baselines for measuring future improvements.',
    },
    {
      step: '04',
      title: 'Engagement',
      subtitle: 'Stakeholder Collaboration',
      description:
        'Hold collaborative workshops with cross-functional teams to validate findings and get input on solutions. This builds buy-in and ensures all perspectives are considered.',
    },
    {
      step: '05',
      title: 'Automation',
      subtitle: 'AI Opportunity Identification',
      description:
        'Identify specific tasks ideal for automation using our expertise in RPA and intelligent automation. We assess feasibility and potential ROI for each opportunity.',
    },
    {
      step: '06',
      title: 'Roadmap',
      subtitle: 'Implementation Plan',
      description:
        'Deliver a data-backed improvement roadmap with prioritized initiatives, automation opportunities, and expected ROI. Includes detailed implementation timelines and resource requirements.',
    },
  ],
  stats: [
    { label: 'Average Efficiency Gain', value: '35%' },
    { label: 'Processes Analyzed', value: '500+' },
    { label: 'Automation Opportunities', value: '78%' },
    { label: 'ROI Within 6 Months', value: '250%' },
  ],
  whyCerebello: [
    {
      title: 'Tailored, Pragmatic Consulting',
      description:
        'We provide practical solutions that are achievable and sustainable given your resources and culture, rather than pushing impractical theories.',
    },
    {
      title: 'Deep Tech + Strategy Expertise',
      description:
        'Unique blend of technological know-how and business strategy acumen. We bridge the gap between strategy and technology seamlessly.',
    },
    {
      title: 'Integration with AI & Automation',
      description:
        'Process Analysis is the first step in your broader automation journey. We design recommendations to dovetail with your AI initiatives.',
    },
    {
      title: 'Core Values',
      description:
        'Guided by Pragmatism, Innovation, Integrity, and Collaboration. We work with your team, not in a silo, co-creating solutions.',
    },
  ],
};

export default function ProcessAnalysisPage() {
  return (
    <>
      <section className="bg-mint-50 py-24 md:py-32">
        <div className="container">
          <div className="mx-auto max-w-4xl">
            <h2 className="text-muted-foreground mb-4 text-2xl font-semibold md:text-3xl">
              {processAnalysisData.title}
            </h2>

            <h1 className="mb-8 max-w-[900px] flex-1 text-3xl leading-13 font-semibold tracking-tight text-balance md:text-4xl lg:text-5xl">
              Uncover How Work Really Happens Inside Your Business
            </h1>

            <div className="space-y-6 text-lg leading-relaxed">
              <p>{processAnalysisData.description}</p>

              <p>{processAnalysisData.introduction}</p>

              <p>{processAnalysisData.content}</p>

              <div className="bg-mint-100 mt-8 rounded-lg p-6">
                <p className="text-mint-900 text-xl font-medium">
                  {processAnalysisData.callToAction}
                </p>
                <Button
                  variant="outline"
                  size="lg"
                  className="border-foreground/20 mt-6 rounded-sm border"
                >
                  <Link href="/contact" className="">
                    <span className="flex items-center gap-2 text-start whitespace-pre-wrap">
                      Book Free Consultation
                      <ArrowRight className="size-4 stroke-3" />
                    </span>
                  </Link>
                </Button>
              </div>
            </div>
          </div>
        </div>
      </section>

      <Feature187
        title="Our Proven Methodology"
        description="Follow our systematic approach to process analysis and optimization"
        steps={processAnalysisData.methodology}
      />
      <Stats9
        title="Proven Results Across Industries"
        description="Our process analysis delivers measurable improvements for organizations worldwide"
        badge="Performance Metrics"
        stats={processAnalysisData.stats}
        benefits={[
          {
            title: 'Process Optimization',
            description:
              'Streamline workflows for maximum efficiency and reduced waste',
          },
          {
            title: 'Bottleneck Identification',
            description: 'Pinpoint where work gets stuck and eliminate delays',
          },
          {
            title: 'Automation Readiness',
            description:
              'Identify and prioritize opportunities for AI and automation',
          },
          {
            title: 'Data-Driven Insights',
            description:
              'Evidence-based recommendations for sustainable improvement',
          },
        ]}
      />
      <Testimonial8 />
      <Cta7
        title="Ready to Transform Your Business?"
        subtitle="Discover Hidden Efficiencies"
        features={[
          'Free Consultation',
          'Custom Solution Design',
          'Proven Methodologies',
          'Ongoing Support',
        ]}
        buttonText="Schedule Consultation"
        buttonHref="/contact"
      />
    </>
  );
}
