    ...
    <section className="py-32">
      <div className="container">
        <div className="mb-16 max-w-xl">
          <Badge
            variant="outline"
            className="bg-muted mb-6 text-sm font-normal"
          >
            Get started today
          </Badge>
        ...
        <Tabs defaultValue="0">
          <TabsList className="mb-10 grid h-full grid-cols-1 gap-8 bg-transparent md:grid-cols-4">
            {integrations.map((item, index) => (
              <TabsTrigger
                ...
                className="md:hover:bg-muted-foreground/5 block cursor-pointer rounded-lg p-4 whitespace-normal"
                onClick={() => setActiveTabId(index)}
              >
                <div className="flex flex-row items-start gap-6 md:flex-col md:items-stretch">
                  <div className="relative flex items-center justify-start">
                    <div
                      className={`border-muted-foreground/10 flex h-7 w-7 items-center justify-center rounded-full border-2 border-solid ${activeTabId === index && 'md:bg-indigo-600 md:text-white md:shadow-[0px_0px_10px_rgba(0,0,255,0.3),0px_0px_25px_rgba(0,0,255,0.1)]'}`}
                    >
                      {index + 1}
                    </div>
                    <div className="bg-muted-foreground/5 absolute left-7 hidden h-[2px] w-[calc(100%+32px)] md:block" />
                    <div className="bg-muted-foreground/5 absolute top-[26px] left-[13px] block h-full w-[2px] md:hidden" />
                  </div>
                ...
                <div className="border-muted-foreground/25 bg-muted-foreground/20 mt-6 block aspect-[1.5] w-full rounded-lg border border-solid p-10 md:hidden">
                  <img
                    src={integrations[index].image}
                    alt={integrations[index].title}
                    className="border-muted-foreground/25 h-full w-full rounded-lg border border-solid object-cover"
                  />
                </div>
              </TabsTrigger>
            ))}
          ...
          <TabsContent
            ...
            className="border-muted-foreground/15 bg-muted-foreground/10 hidden aspect-[1.5] w-full rounded-lg border border-solid p-10 md:block"
          >
            <img
