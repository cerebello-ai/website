...
const PlanCard = ({ plan }: { plan: PricingPlan }) => {
  return (
    ...
    >
      ...
      <div className="text-muted-2-foreground text-xs">
        <div>per person / month</div>
        <div>{plan.bestFor}</div>
      </div>
      <div className="text-foreground mt-4 mb-6 text-lg font-medium">
        {plan.description}
      </div>
      <Button
        asChild
      ...
      {plan.mostPopular && (
        <div className="bg-primary text-primary-foreground absolute top-0 left-1/2 w-fit -translate-1/2 -translate-y-1/2 rounded-full px-4 py-1 text-xs font-medium">
          Most popular
        </div>
      )}
    </div>
