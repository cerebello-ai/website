...
const Blog1 = ({
  ...
  gridColumns = 'md:grid-cols-2 2xl:grid-cols-3',
  showFilters = true,
}: Blog1Props) => {
  return (
    <section className="py-32">
      <div className="container">
          ...
          {showFilters && (
            <div className="mb-10 flex flex-wrap items-center gap-x-4 gap-y-3 lg:gap-x-3">
              {filters.map((filter, index) => (
                <div key={`filter-${index}`} className="shrink-0">
                  <Select>
                    ...
                    <SelectContent>
                      {filter.values.map((value, valueIndex) => (
                        <SelectItem
                          key={`filter-value-${index}-${valueIndex}`}
                          value={value}
                        >
                          {value}
                        </SelectItem>
        ...
        <div className={`grid gap-4 lg:gap-6 ${gridColumns}`}>
          {posts.map((post) => (
            <a
              key={post.id}
              href={post.href}
